...
fn move_pieces(time: Res<Time>, mut query: Query<(&mut Transform, &Piece)>) {
    for (mut transform, piece) in query.iter_mut() {
        ...
        // Move if piece isn't there
        if direction.length() > 0.1 {
            transform.translation += 3. * direction.normalize() * time.delta_seconds();
        }
    }
}

// If square empty, none. Else, color of piece occupying it. 
...
fn spawn_king(
    ...
    // Some of meshes are offset, so we'll use a parent entity to track actual 
    // position and child to track mesh. 
    // Proper way is to just edit the meshes. 
    commands
        // Spawn parent entity
        .spawn(PbrBundle {
            transform: Transform::from_translation(Vec3::new(
                position.0 as f32, 0., position.1 as f32,
            )),
            ..Default::default()
        })
        .with(Piece {
...
fn create_pieces(commands: &mut Commands, asset_server: Res<AssetServer>, 
